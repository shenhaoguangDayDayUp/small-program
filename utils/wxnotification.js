...
function addNotification(name, selector, observer) {
  if (name && selector) {
    if (!observer) {
      isDebug && console.log("addNotification Warning: no observer will can't remove notice");
    }
    isDebug && console.log("addNotification:" + name);
    var newNotice = {
      name: name,
      selector: selector,
      observer: observer
    ...
    isDebug && console.log("addNotification error: no selector or name");
  }
}
...
function removeNotification(name, observer) {
  isDebug && console.log("removeNotification:" + name);
  for (var i = 0; i < __notices.length; i++) {
    var notice = __notices[i];
    if (notice.name === name) {
      if (notice.observer === observer) {
...
function postNotificationName(name, info) {
  isDebug && console.log("postNotificationName:" + name);
  if (__notices.length == 0) {
    isDebug && console.log("postNotificationName error: u hadn't add any notice.");
    return;
  }
...
/// 用户登录成功的通知名称
let kAppLoginSuccess = "AppLoginSuccess";
/// 业务APP登录失败
let kAppLoginError = "AppLoginError";
/// 用户城市定位成功
let kUserLocationSuccess = "UserLocationSuccess";
/// 获取用户信息成功
let kAppUserInfoSuccess = "AppUserInfoSuccess";
/// 获取用户信息失败
let kAppUserInfoError = "AppUserInfoError";
...
module.exports = {
  ...
  kAppLoginSuccess: kAppLoginSuccess,
  kAppLoginError: kAppLoginError,
  kUserLocationSuccess: kUserLocationSuccess,
  kAppUserInfoSuccess: kAppUserInfoSuccess,
  kAppUserInfoError: kAppUserInfoError,
}
