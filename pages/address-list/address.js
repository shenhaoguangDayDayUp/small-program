...
Page({
   ...
   */
  onUnload: function () {
    WxNotificationCenter.removeNotification('ADDRESSUPDAT', this);
  ...
  selectEdit(value){
    ajax('member/receivers/favorite/{code}', value.detail.code ).put().then(res=>{
      WxNotificationCenter.postNotificationName('ADDRESSUPDAT')

      pageTo('../convert/convert', { productList:this.data. productList},true)
    })
  },
  gotoEdit(value) {
