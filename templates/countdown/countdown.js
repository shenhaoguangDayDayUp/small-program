class Countdown {
	 ...
	 */
	render() {
    let diffDate = this.getDiffDate();
    let years = diffDate.years;
    let days = diffDate.days;
    let hours = diffDate.hours;
    let min = diffDate.min;
    let sec = diffDate.sec;
    if (years <= 0 && days <= 0 && hours <= 0 && min <= 0 && sec <= 0){
      this.options.onEnd && this.options.onEnd();
      return;
    }
    this.options.render(diffDate)
		return this
	}
